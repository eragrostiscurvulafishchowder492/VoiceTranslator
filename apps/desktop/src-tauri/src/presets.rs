...
pub fn builtin_presets() -> Vec<(String, String)> {
    // (name, graph_json)
    let mut out = Vec::new();
    ...
    out.push(graph(
        ...
        vec![
            edge("mic", "out", "gain", "in"),
            edge("gain", "out", "lim", "in"),
            edge("lim", "out", "out", "in"),
        ],
    ));
    ...
    out.push(graph(
        "中文 ASR 字幕",
        "麦克风 → VAD → 流式识别 → 字幕面板（text.file_output 记录）",
        vec![
            node(
                "mic",
                "audio.microphone",
            ...
            node(
                ...
                820.0,
                160.0,
                json!({"path": "subtitles.txt"}),
            ),
        ],
        vec![
            edge("mic", "out", "rs", "in"),
            edge("rs", "out", "asr", "in"),
            edge("asr", "final", "txt", "in"),
        ],
    ));
    ...
    out.push(graph(
        ...
        vec![
            ...
            edge("norm", "out", "ttsreq", "in"),
            edge("ttsreq", "out", "tts", "in"),
            edge("tts", "out", "lim", "in"),
            edge("lim", "out", "vout", "in"),
        ],
    ));
    ...
    out.push(graph(
        ...
        vec![
            edge("file", "out", "gain", "in"),
            edge("gain", "out", "vout", "in"),
            edge("gain", "out", "sout", "in"),
        ],
    ));
    ...
    out.push(graph(
        "中文翻译语音 (实验)",
        "需安装翻译插件（text.translator）后把『标准化』连到翻译节点；当前模板显示缺失依赖",
        vec![
            node(
                "mic",
                "audio.microphone",
            ...
            node(
                ...
                780.0,
                160.0,
                json!({"path": "translate_in.txt"}),
            ),
        ],
        vec![
            edge("mic", "out", "rs", "in"),
            edge("rs", "out", "asr", "in"),
            edge("asr", "final", "note", "in"),
        ],
    ));
