...
use voice_pipeline_core::runtime::{ExecutionEngine, PluginBridge, PluginOutputRouter};
...
pub fn save_pipeline(
    ...
    let g: PipelineGraph =
        serde_json::from_str(&graph_json).map_err(|e| format!("图格式错误: {e}"))?;
    app.store
        .save_pipeline(&g.id, &name, &graph_json, is_default)
        .map_err(|e| e.to_string())?;
    app.store.set_last_known_good(&graph_json).ok();
    app.logring
        .push("INFO", "pipeline", format!("已保存管线 {name}"));
    Ok(g.id)
}
...
pub fn start_pipeline_impl(app: &AppState, g: PipelineGraph) -> Result<serde_json::Value, String> {
    ...
    for pid in &needed {
        ...
        match st {
            Some(s) if s.enabled && s.state != "running" => {
                app.logring
                    .push("INFO", "plugin", format!("自动启动插件 {pid}"));
    ...
    let mic_key = mic_node
        .and_then(|n| n.params.get("device"))
        .and_then(|v| v.as_str())
        .unwrap_or("")
        .to_string();
    let out_key = out_node
        .and_then(|n| n.params.get("device"))
        .and_then(|v| v.as_str())
        .unwrap_or("")
        .to_string();
    ...
    if let Some(n) = mic_node {
        let key = if mic_key.is_empty() {
            voice_audio_engine::devices::list_devices()
                .into_iter()
                .find(|d| d.default_input)
                .map(|d| d.key)
                .unwrap_or_default()
        } else {
            mic_key.clone()
    ...
    if out_node.is_some() {
        let key = if out_key.is_empty() {
            voice_audio_engine::devices::find_virtual_output()
                .map(|d| d.key)
                .or_else(|| {
                    voice_audio_engine::devices::list_devices()
                        .into_iter()
                        .find(|d| d.default_output)
                        .map(|d| d.key)
                })
                .unwrap_or_default()
        } else {
    ...
    let mut bridges: std::collections::HashMap<String, Arc<dyn PluginBridge>> =
...
pub fn pipeline_snapshot(app: App) -> Result<serde_json::Value, String> {
    ...
    Ok(g.as_ref()
        .map(|e| e.snapshot())
        .unwrap_or(json!({"state": "Stopped", "running": false})))
}
...
pub fn pipeline_control(app: App, signal: String) -> Result<(), String> {
    match signal.as_str() {
        ...
        "mute_on" => {
            app.muted.store(true, Ordering::Relaxed);
            let g = app.playback.lock();
            if let Some(p) = g.as_ref() {
                p.set_muted(true);
            }
        }
        "mute_off" => {
            app.muted.store(false, Ordering::Relaxed);
            let g = app.playback.lock();
            if let Some(p) = g.as_ref() {
                p.set_muted(false);
            }
...
pub fn plugin_prepare_env(app: App, id: String) -> Result<serde_json::Value, String> {
    ...
    app.rebuild_registry();
    app.logring
        .push("INFO", "plugin", format!("环境创建完成 {id} ({ms}ms)"));
...
pub fn plugin_install(app: App, zip_path: String) -> Result<String, String> {
    let id = app
        .plugins
        .install_zip(std::path::Path::new(&zip_path))
        .map_err(|e| e.to_string())?;
    app.rebuild_registry();
    Ok(id)
}
...
pub fn settings_set(
    ...
    app.store
        .set_setting("ui_settings", &settings.to_string())
        .map_err(|e| e.to_string())?;
    // 热键即时重注册
    ...
    app.logring
        .push("INFO", "hotkey", format!("热键已重注册: {registered:?}"));
    Ok(())
}
...
pub fn test_tts(
    ...
    let bridge = app.plugins.ensure_bridge(&plugin_id).ok_or("插件未安装")?;
    let st = app
        .plugins
        .list_status()
        .into_iter()
    ...
    let c2 = col.clone();
    let router: Arc<PluginOutputRouter> = Arc::new(move |_node: &str, _port: &str, msg: Msg| {
        if let Payload::Audio(a) = msg.payload {
            let mut g = c2.lock();
            if g.rate == 0 || !a.samples.is_empty() {
                g.rate = a.sample_rate;
            }
            if g.ttfa_ms == 0 && !a.samples.is_empty() {
                g.ttfa_ms = g.started.elapsed().as_millis() as u64;
            }
            let bytes: &[u8] = unsafe {
                std::slice::from_raw_parts(a.samples.as_ptr() as *const u8, a.samples.len() * 4)
            };
            g.samples.extend(
                bytes
                    .chunks_exact(4)
    ...
    let msg = Msg {
        ...
        payload: Payload::TtsRequest(voice_pipeline_core::types::TtsRequestMsg {
            request_id: format!("lab{ts}"),
            text: text.clone(),
            language: "zh".into(),
            voice_profile: "default".into(),
            style: "calm".into(),
            speed: 1.0,
            pitch: 1.0,
            energy: 1.0,
    ...
    bridge
        .send(&instance_id, msg)
        .map_err(|e| format!("发送失败: {e}"))?;
    let t_start = std::time::Instant::now();
    let got_done = done_rx
