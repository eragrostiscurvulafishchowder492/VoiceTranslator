...
fn node(id: &str, ty: &str, label: &str, params: serde_json::Value) -> NodeInstance {
    NodeInstance { id: id.into(), node_type: ty.into(), label: label.into(), params,
                   position: (0.0, 0.0), bypassed: false, notes: String::new(), group: String::new() }
}
...
fn sim(a: &str, b: &str) -> f64 {
    ...
    for i in 1..=n {
        for j in 1..=m {
            cur[j] = if a[i - 1] == b[j - 1] { prev[j - 1] + 1 } else { cur[j - 1].max(prev[j]) };
        }
...
fn main() {
    ...
    assert!(speech.exists(), "测试语音缺失: {SPEECH_WAV}（先跑 tests/smoke_ai_pipeline.py）");
    ...
    // ---------- T1 变声 ----------
    {
        ...
        let g = graph("t1-vc", vec![
            node("f", "audio.file", "文件", json!({"path": speech.to_string_lossy(), "loop": false})),
            node("vc", "org.voicestudio.vcpitch/vcpitch.voice_convert", "变声", json!({"character": "female_bright"})),
            node("r", "audio.file_recorder", "录制", json!({"path": out.to_string_lossy()})),
        ...
        match start_pipeline_impl(&state, g) {
            Ok(_) => {
                ...
                std::thread::sleep(Duration::from_secs((orig0.len() as f64 / 48000.0) as u64 + 5));
                ...
                match voice_audio_engine::wav::read_mono_48k(&out) {
                    Ok((samples, _, _)) if samples.len() > 24000 => {
                        ...
                        if z2 > z1 * 1.15 && (0.8..1.25).contains(&dur_ratio) {
                            ...
                            println!("  FAIL：ZCR {z1:.3}→{z2:.3}，时长比 {dur_ratio:.2}"); fail += 1;
                        }
                    }
                    _ => { println!("  FAIL：输出缺失/过短"); fail += 1; }
                }
            }
    ...
    // ---------- T2 ASR ----------
    {
        ...
        let tx2 = tx.clone();
        let g = graph("t2-asr", vec![
            node("f", "audio.file", "文件", json!({"path": speech.to_string_lossy(), "loop": false})),
            node("rs", "audio.resampler", "重采样", json!({"target_rate": 16000})),
            node("asr", "org.voicestudio.funasr/funasr.streaming_asr", "识别", json!({})),
        ], vec![edge("f", "out", "rs", "in"), edge("rs", "out", "asr", "in")]);
        match start_pipeline_impl(&state, g) {
            Ok(_) => {
                // 接管路由收集识别文本（ASR 无下游边）
                if let Some(b) = state.plugins.ensure_bridge("org.voicestudio.funasr") {
                    b.set_router(Arc::new(move |_n, _p, m: Msg| { let _ = tx2.send(m); }));
                }
                    ...
                    match rx.recv_timeout(Duration::from_millis(500)) {
                        Ok(m) => {
                            if let Payload::Text(t) = m.payload {
                                if t.is_final { final_text = t.text; break; }
                            }
                        }
                        Err(crossbeam_channel::RecvTimeoutError::Timeout) => {
                            let done = state.engine.lock().as_ref()
                                .map(|e| e.snapshot()["nodes"].get("asr").map(|s| s == "Ready").unwrap_or(false))
                                .unwrap_or(false);
                ...
                if s >= 0.7 {
                    println!("  PASS：识别=\"{final_text}\" 相似度 {s:.2}"); pass += 1;
                } else {
                    println!("  FAIL：识别=\"{final_text}\" 相似度 {s:.2}");
                    for st in state.plugins.list_status().iter().filter(|x| x.id.contains("funasr")) {
                        println!("       插件状态: {} detail={}", st.state, st.detail);
                    }
                    for l in state.logring.snapshot(12).iter().filter(|l| l.component != "audio") {
    ...
    if !skip_full {
        ...
        let _ = std::fs::remove_file(&out);
        let g = graph("t3-full", vec![
            node("f", "audio.file", "文件", json!({"path": speech.to_string_lossy(), "loop": false})),
            node("rs", "audio.resampler", "重采样", json!({"target_rate": 16000})),
            node("asr", "org.voicestudio.funasr/funasr.streaming_asr", "识别", json!({})),
            node("seg", "org.voicestudio.textkit/textkit.segmenter", "断句", json!({"stable_rounds": 2, "flush_timeout_ms": 800})),
            node("norm", "org.voicestudio.textkit/textkit.normalizer", "标准化", json!({"mode": "gaming"})),
            node("tt", "org.voicestudio.textkit/textkit.to_tts", "→TTS", json!({})),
            node("tts", "org.voicestudio.cosyvoice/cosyvoice.zero_shot_tts", "TTS", json!({})),
            node("lim", "audio.limiter", "限幅", json!({})),
            node("r", "audio.file_recorder", "录制", json!({"path": out.to_string_lossy()})),
        ], vec![
            edge("f", "out", "rs", "in"), edge("rs", "out", "asr", "in"),
            edge("asr", "partial", "seg", "in"), edge("seg", "out", "norm", "in"),
            edge("norm", "out", "tt", "in"), edge("tt", "out", "tts", "in"),
            edge("tts", "out", "lim", "in"), edge("lim", "out", "r", "in"),
        ]);
        match start_pipeline_impl(&state, g) {
            Ok(_) => {
                ...
                if let Some(b) = state.plugins.ensure_bridge("org.voicestudio.textkit") {
                    let _ = b.send("seg", Msg {
                        ...
                        payload: Payload::Control(voice_pipeline_core::types::ControlMsg {
                            signal: "flush".into(), payload_json: "{}".into() }),
                    });
                }
                // 等录制完成：文件尺寸稳定
                let t0 = Instant::now();
                ...
                while t0.elapsed() < Duration::from_secs(240) {
                    ...
                    if len == last_len && len > 0 { stable += 1; } else { stable = 0; }
                    last_len = len;
                    if stable >= 10 { break; }
                }
                stop_pipeline_impl(&state);
                match voice_audio_engine::wav::read_mono_48k(&out) {
                    Ok((samples, _, _)) => {
                        ...
                        if dur > 1.0 && rms > 0.01 && peak <= 1.0 {
                            ...
                            println!("  FAIL：dur={dur:.2}s rms={rms:.3} peak={peak:.3}"); fail += 1;
                        }
                    }
                    _ => { println!("  FAIL：无输出 WAV"); fail += 1; }
                }
            }
            Err(e) => { println!("  FAIL：{e}"); fail += 1; }
        }
    ...
    // ---------- T4 原声监听 ----------
    {
        ...
        let g = graph("t4-monitor", vec![
            node("m", "audio.microphone", "麦克风", json!({"block_ms": 20, "gain_db": 3.0})),
        ...
        match start_pipeline_impl(&state, g) {
            Ok(_) => {
                ...
                let snap = state.engine.lock().as_ref().map(|e| e.snapshot()).unwrap_or(json!({}));
                stop_pipeline_impl(&state);
                let edge_sent: u64 = snap["edges"].as_array().map(|a|
                    a.iter().filter_map(|e| e["sent"].as_u64()).sum()).unwrap_or(0);
                ...
                if edge_sent > 200 && underruns == 0 {
                    ...
                    println!("  FAIL：edge_sent={edge_sent} underrun={underruns} overflow={overflows}"); fail += 1;
                }
            }
            Err(e) => { println!("  FAIL：{e}"); fail += 1; }
        }
