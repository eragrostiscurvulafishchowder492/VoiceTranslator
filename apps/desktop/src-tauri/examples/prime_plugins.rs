...
fn main() {
    let only: Option<String> = std::env::args().skip(1)
        .skip_while(|a| a != "--only").nth(1);
    ...
    let light = ["org.voicestudio.textkit", "org.voicestudio.vcpitch",
                 "org.voicestudio.gain", "org.voicestudio.textreplace",
                 "org.voicestudio.nullout", "org.voicestudio.extcmd",
                 "org.voicestudio.tonegen"];
    let mut ok = 0;
    for id in light {
        if let Some(o) = &only { if !id.contains(o) { continue; } }
        ...
        if !state.plugins.env_exists(id) {
            match state.plugins.prepare_env(id) {
                ...
                Err(e) => { println!("env 创建失败 {id}: {e}"); continue; }
            }
        }
        print!("prime {id} ... ");
        match state.plugins.start_plugin(id) {
            Ok(_) => { state.rebuild_registry(); state.plugins.stop_plugin(id).ok(); println!("OK"); ok += 1; }
            Err(e) => println!("失败: {e}"),
        }
    }
    state.rebuild_registry();
    println!("完成：{ok} 个插件节点已缓存；注册表现有 {} 个节点", state.registry.read().all().len());
}
