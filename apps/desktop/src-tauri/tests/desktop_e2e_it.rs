...
//! 需 .venv（无则跳过）。
use std::sync::Arc;
use voice_pipeline_core::graph::{NodeInstance, PipelineGraph, Edge};
...
fn plugin_node_pipeline_e2e_text_to_tts_request() {
    ...
    g.nodes.push(n("t", "text.manual_input", "文本输入",
        serde_json::json!({"text": "你们先过去，我拿一下东西，马上回来。"})));
    g.nodes.push(n("tt", "org.voicestudio.textkit/textkit.to_tts", "→TTS请求",
        serde_json::json!({"voice_profile": "default", "speed": 1.0})));
    g.edges.push(Edge { id: "e1".into(), from_node: "t".into(), from_port: "out".into(),
                        to_node: "tt".into(), to_port: "in".into(),
                        backpressure: Default::default(), capacity: 8 });

    // —— start_pipeline 的核心语义（同构复制，因 tauri::State 无法在测试构造）——
    state.plugins.start_plugin("org.voicestudio.textkit")
        .expect("textkit 自动启动");
    for node in &g.nodes {
        if node.node_type.contains('/') {
            ...
            state.plugins.configure_node(pid, short, &node.id, &params)
                .expect("Configure 下发");
        }
    }
    ...
    bridge.set_router(Arc::new(move |_node: &str, _port: &str, m: Msg| { let _ = tx2.send(m); }));
