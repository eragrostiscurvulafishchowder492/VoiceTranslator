...
fn voice_audio_engine_copy_dir(src: &std::path::Path, dst: &std::path::Path) {
    ...
    for e in std::fs::read_dir(src).unwrap().flatten() {
        ...
        if e.path().is_dir() {
            voice_audio_engine_copy_dir(&e.path(), &to);
        } else {
...
fn full_plugin_lifecycle_with_real_worker() {
    ...
    let st = mgr
        .list_status()
        .into_iter()
        .find(|s| s.id == "org.voicestudio.gain")
        .unwrap();
    assert_eq!(st.state, "running");
    assert!(st.pid.is_some(), "独立 worker 进程应有 pid");
    assert!(
        st.node_types.contains(&"gain.audio_gain".to_string()),
        "握手应返回节点类型: {:?}",
        st.node_types
    );
    ...
    assert!(specs
        .iter()
        .any(|s| s.node_type == "org.voicestudio.gain/gain.audio_gain"));
    ...
    let tx2 = tx.clone();
    bridge.set_router(Arc::new(move |_n: &str, _p: &str, m: Msg| {
        let _ = tx2.send(m);
    }));
    // Configure
    {
    ...
    if let Payload::Audio(a) = got.payload {
        ...
        assert!(
            (a.samples[0] - 0.25).abs() < 0.01,
            "0dB 应原样: {}",
            a.samples[0]
        );
    } else {
        panic!("应收到音频，得到 {:?}", got.payload.port_type());
    }
