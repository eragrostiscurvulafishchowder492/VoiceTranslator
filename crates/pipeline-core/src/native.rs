...
pub type PullInputCallback = dyn Fn(&mut [f32]) -> usize + Send + Sync;
pub type SubmitOutputCallback = dyn Fn(&[f32]) + Send + Sync;

/// 宿主注入的设备桥：mic 源 / 输出汇，避免管线核心直接依赖具体音频实现。
pub struct DeviceBridge {
    /// 拉取 48k mono 输入（返回实际帧数）
    pub pull_input: Box<PullInputCallback>,
    /// 提交 48k mono 输出（主输出）
    pub submit_output: Box<SubmitOutputCallback>,
    /// 当前 PTT 按下状态
...
pub fn builtin_registry() -> NodeRegistry {
    ...
    let entries: Vec<NodeSpec> = vec![
        ...
        NodeSpec {
            ...
            category: "控制".into(),
            inputs: vec![
                ps("in", PortType::AudioPcm, true),
                ps("control", PortType::ControlSignal, false),
            ],
            outputs: audio_out(),
        ...
        NodeSpec {
            ...
            category: "控制".into(),
            inputs: vec![
                ps("in", PortType::AudioPcm, false),
                ps("text", PortType::TextPartial, false),
            ],
...
pub enum NativeProcessor {
    ...
    Resampler {
        target: u32,
        rs: Box<voice_audio_engine::resampler::StreamResampler>,
        seq: u64,
    },
    ChannelConverter {
        target: u16,
...
pub fn create_native(
    ...
    Ok(match node_type {
        ...
        "audio.file" => {
            ...
            let (samples, rate, _) =
                voice_audio_engine::wav::read_mono_48k(std::path::Path::new(path))?;
        ...
        "audio.noise_gate" => NativeProcessor::NoiseGate(voice_audio_engine::dsp::NoiseGate::new(
            g("threshold_db", -45.0),
            3.0,
            60.0,
            48_000.0,
        ...
        "audio.limiter" => NativeProcessor::Limiter(voice_audio_engine::dsp::Limiter::new(
            g("ceiling_db", -1.0),
            50.0,
            48_000.0,
        )),
        "audio.high_pass" => NativeProcessor::HighPass(voice_audio_engine::dsp::HighPass::new(
            48_000.0,
            g("cutoff_hz", 80.0),
        )),
        "audio.resampler" => NativeProcessor::Resampler {
            ...
            rs: Box::new(voice_audio_engine::resampler::StreamResampler::new(
                48_000, 16_000, 480,
            )?),
            seq: 0,
        },
        ...
        "audio.speaker_output" | "audio.virtual_output" => NativeProcessor::SpeakerOut {
            bridge: bridge.clone(),
        },
        "audio.file_recorder" => {
            let path = params.get("path").and_then(|v| v.as_str()).unwrap_or("");
        ...
        "control.push_to_talk" => NativeProcessor::PushToTalk {
            bridge: bridge.clone(),
        },
        "metrics.tap" => NativeProcessor::MetricsTap {
            count: 0,
            last_ns: 0,
        },
