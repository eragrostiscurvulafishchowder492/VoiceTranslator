...
fn main() {
    ...
    for d in &devs {
        if d.is_input {
            println!(
                "IN  {} (default={}) {}ch {}Hz",
                d.name, d.default_input, d.max_input_channels, d.default_sample_rate
            );
        }
    }
    ...
    while t0.elapsed().as_secs() < 3 {
        ...
        if n > 0 {
            ...
            peak = peak.max(lv.peak);
            sum += lv.rms * lv.rms * n as f32;
            total += n;
        }
    ...
    if rms > 0.001 {
        println!("MIC PASS（检测到环境声）");
    } else {
        println!("MIC OK（设备工作，静音环境）");
    }
}
