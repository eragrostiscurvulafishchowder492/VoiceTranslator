...
impl PlaybackWorker {
    pub fn start(main_key: &str, monitor_key: Option<&str>, gain_db: f32) -> anyhow::Result<Self> {
        ...
        let (stream, ring) = open_output(main_key, muted.clone(), underruns.clone(), gain_db, true)?;
        let (monitor_stream, monitor_ring) = match monitor_key {
            Some(k) => { let (s, r) = open_output(k, muted.clone(), underruns.clone(), gain_db - 6.0, false)?; (Some(s), Some(r)) }
            None => (None, None),
        };
        Ok(Self {
            ring, monitor_ring, stream: Some(stream), monitor_stream,
            muted, underruns,
            device_key: main_key.into(), monitor_key: monitor_key.unwrap_or("").into(),
            cross: Vec::new(),
        })
    }
    ...
    pub fn submit(&mut self, chunk: &[f32]) {
        ...
        if let Some(mr) = &self.monitor_ring { mr.push(&block); }
    }

    pub fn set_muted(&self, m: bool) { self.muted.store(m, Ordering::Relaxed); }
    pub fn is_muted(&self) -> bool { self.muted.load(Ordering::Relaxed) }
    pub fn underrun_count(&self) -> u64 { self.underruns.load(Ordering::Relaxed) }

    pub fn stop(&mut self) {
        if let Some(s) = self.stream.take() { s.pause().ok(); }
        if let Some(s) = self.monitor_stream.take() { s.pause().ok(); }
    }
}

fn open_output(device_key: &str, muted: Arc<AtomicBool>, underruns: Arc<AtomicU64>,
               gain_db: f32, is_main: bool) -> anyhow::Result<(Stream, Arc<RingBuffer>)> {
    ...
    let err_cb = move |e| log::error!("output stream error ({}): {e}", if is_main { "main" } else { "monitor" });
    ...
    let stream = match cfg.sample_format() {
        SampleFormat::F32 => dev.build_output_stream(
            ...
            move |data: &mut [f32], _: &OutputCallbackInfo| {
                render(data, ch, &ring_r, &muted, gain, &mut rs, &mut leftover, &mut need, &underruns)
            }, err_cb, None)?,
        SampleFormat::I16 => dev.build_output_stream(
            ...
            move |data: &mut [i16], _: &OutputCallbackInfo| {
                ...
                render(&mut tmp, ch, &ring_r, &muted, gain, &mut rs, &mut leftover, &mut need, &underruns);
                for (o, s) in data.iter_mut().zip(tmp) { *o = (s * 32767.0) as i16; }
            }, err_cb, None)?,
...
fn render(data: &mut [f32], ch: u16, ring: &RingBuffer, muted: &AtomicBool, gain: f32,
          rs: &mut crate::resampler::StreamResampler, leftover: &mut Vec<f32>,
          need: &mut usize, underruns: &AtomicU64) {
    let frames = data.len() / ch as usize;
    while leftover.len() < frames {
        let mut src = vec![0f32; 960.min(2048)];
        let got = ring.pop(&mut src);
        if got == 0 {
            if !leftover.is_empty() { break; }
    ...
    let m = if muted.load(Ordering::Relaxed) { 0.0 } else { gain };
